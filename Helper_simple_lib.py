...
def drawdown(df=None):
    ...
    p = np.cumprod((1+df/100).values, axis=0)
...
def Return_DD(markers=["o", "v", "*", "X"],
    ...
    -------
        ...
        - ylim: list indicating the maximum and minimum for the drawdown plot
    ...
    df_cum = np.cumprod(1+ df.loc[start:end, :] / 100)
    # index same for all
    ...
    for i, c in enumerate(df.loc[:, factor].columns):
        ...
        if c == 'Mkt-RF' and final_value:
            ax[0].annotate('$%0.2f' % df_cum.loc[end, c], xy=(1, df_cum.loc[end, c]-50), xytext=(8, 0), 
                 xycoords=('axes fraction', 'data'), textcoords='offset points')
        elif c == 'WML' and final_value:
            ax[0].annotate('$%0.2f' % df_cum.loc[end, c], xy=(1, df_cum.loc[end, c]+10), xytext=(8, 0), 
                 xycoords=('axes fraction', 'data'), textcoords='offset points')
        elif final_value:
            ax[0].annotate('$%0.2f' % df_cum.loc[end, c], xy=(1, df_cum.loc[end, c]), xytext=(8, 0), 
                 xycoords=('axes fraction', 'data'), textcoords='offset points')
    ...
    def visualize(self, size=(18, 18), nr_paths=50, last_15=None):
        ...
        for key, value in self.simulated_paths.items():
            ...
            if last_15 is not None:
                ...
                # simulated path
                sim_prices = np.cumprod(1+value/100)
                # observed path
            ...
            # plot observed path
            observed_price = (1+observed_path / 100).reset_index(
                drop=True).cumprod()
...
def append_stats(stat=None, dic=None):
    """
...
def permutation_plot(statistic='SR',
                     period="1927-2019",
                     statistic_name="observed statistic",
                     color_title="gray",
                     y_title=1.025,
                     add_legend=False,
                     hist=False,
    ...
    for f, df_f in df_stat.groupby(["Strategy"], sort=False):
        ...
        axes[2].set_xlabel(xlabel=statistic if xlab is None else xlab, fontsize=16)
        axes[3].set_xlabel(xlabel=statistic if xlab is None else xlab, fontsize=16)
        axes[0].set_ylabel(ylabel="Density", fontsize=16)
        axes[2].set_ylabel(ylabel="Density", fontsize=16)
    ...
    if add_legend:
        ...
        if dic is not None:
            ...
            nr_s_methods = len(df["Strategy"].unique())
        axes[1].legend(
